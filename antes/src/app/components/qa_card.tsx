...
export const QACard: FC<QACardProps> = ({ qa }) => {
  const context = { 
  ...
  return (
    <div className='flex flex-col justify-between w-[525px] gap-5 p-7 rounded-lg bg-background shadow-cbs text-base font-font1 text-font1'>
      ...
      <div className='flex flex-col gap-3'>
        ...
        <p className='text-extra line-clamp-3 lg:line-clamp-6'>{context.description}</p>
      </div>
      ...
      <div className='flex lg:flex-row flex-col lg:items-center justify-between gap-5'>
        ...
        <Link href={context.link} className='flex flex-row items-center justify-center gap-2 px-4 py-3 rounded-lg bg-primary text-font2 font-semibold text-sm'>Lees meer <ArrowForwardRounded className='fill-font2 h-5'/></Link>
      </div>
    </div>
  );
};