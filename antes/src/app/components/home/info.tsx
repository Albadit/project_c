...
const context = {
  ...
  link: [
    { url: "#", title: "Bekijk de vacatures", link_text: "Antes vacaturebank", font_color: "text-font2", background_color: "bg-callToAction", fill_color:"fill-font2"},
    { url: "#", title: "Kom meer te weten over Antes", link_text: "Stel hier je vragen", font_color: "text-font2", background_color: "bg-callToAction", fill_color:"fill-font2"},
    { url: "#", title: "Neem een kijk bij een Antes in jouw buurt", link_text: "Plan een afspraak", font_color: "text-callToAction", background_color: "bg-font2", fill_color:"fill-callToAction"},
  ]
}
    ...
    <div className='flex justify-between flex-col min-h-[600px] lg:w-full w-[525px] gap-10 p-7 rounded-lg bg-info bg-no-repeat bg-cover bg-center shadow-cbs text-base font-font1 text-font2'>
      ...
      {context.link.map((item) => ( 
        <Link href={item.url} className={`flex flex-col items-start justify-between w-full gap-2 px-4 py-3 rounded-lg font-semibold ${item.background_color} ${item.font_color}`}>
          ...
          <p className='flex flex-row items-center justify-center gap-2 font-semibold text-sm'>{item.link_text} <ArrowForwardRounded className={`h-5 ${item.fill_color}`}/></p>
        </Link>
      ))}
      </div>
    </div>
