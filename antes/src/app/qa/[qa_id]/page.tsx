  ...
  return (
    ...
    {session && status === "authenticated" ? (<NavDashboard user={session?.user}/>) : (<NavHome />)}
      <main className='m-auto p-5 my-12 max-w-[1000px]'>
        {isLoading ? (<p className='text-center'>Loading data...</p>) : ( data?.status === "error" ? (<p className='text-center'>No Q&A data</p>) : (
          <section className='flex flex-col w-full gap-5 font-font2'>
          <h1 className='font-font1 font-bold text-primary text-5xl'>{data?.creator.title}</h1>
          <div className='flex flex-col text-extra'>
            <p>Gevraagd: {data?.creator.name}</p>
            <p>Datum: {formatDate(data?.creator.dateCreate ?? '')}</p>
          </div>
          <hr />
          {data?.qaAnswers.map((item, index) => (
            <div key={index} className={`flex ${session?.user?.email == item.email ? 'flex-row-reverse justify-start' : 'flex-row justify-start'} sm:gap-5 gap-2`}>
              <img src={item.image} alt="profile" className='h-full block'/>
          ...
          {session && status === "authenticated" ? (
            <div className='flex flex-row gap-5 max-w-[565px] w-full'>
            <form onSubmit={handleSubmit} className='comment bg-section flex flex-col gap-2 p-3 w-full h-[120px] rounded-lg shadow-sm border-[1px] border-font1/20 focus-within:outline focus-within:outline-2 focus-within:outline-callToAction focus-within:outline-offset-[-2px] sm:text-sm sm:leading-6 disabled:bg-slate-50 disabled:text-text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500 disabled:opacity-75'>
